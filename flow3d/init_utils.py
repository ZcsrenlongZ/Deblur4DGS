...
def init_fg_from_tracks_3d(
    ...
    print('fore ground', tracks_3d.xyz.shape, 'cano_t', cano_t)  # fore ground torch.Size([22200, 24, 3]) cano_t 7
...
def init_bg(
    ...
    print(bg_means.shape, bg_quats.shape, bkdg_scales.shape, bkdg_colors.shape, bg_opacities.shape)
    return gaussians
...
def init_motion_params_with_procrustes(
    ...
    means_cano = means_cano[valid_mask]          # 用于初始化 can_t space
    ...
    # ids = ids[counts > 100]  #  如果一个类的点太少，就移除这个bases
    num_bases = len(ids)
    ...
    # 用于把points从cno_t转到其他帧下  
    # init_rots.shape=torch.Size([10, 24, 6]), init_ts.shape=torch.Size([10, 24, 3]) [22200, 10]
    ...
    print(f'init_rots: {init_rots.shape}, init_ts: {init_ts.shape}')
