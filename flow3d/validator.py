...
class Validator:
    def __init__(
        ...
        self.psnr_metric = mPSNR().to(device)
        self.ssim_metric = mSSIM().to(device)
        self.lpips_metric = mLPIPS().to(device)
        self.fg_psnr_metric = mPSNR().to(device)
        self.fg_ssim_metric = mSSIM().to(device)
        self.fg_lpips_metric = mLPIPS().to(device)
        self.bg_psnr_metric = mPSNR().to(device)
        self.bg_ssim_metric = mSSIM().to(device)
        self.bg_lpips_metric = mLPIPS().to(device)
        self.pck_metric = PCK().to(device)
    ...
    @torch.no_grad()
    def validate(self, epoch, mode="mid"):
        self.reset_metrics()
        metric_imgs, results = self.validate_imgs(epoch, mode=mode)
        return {**metric_imgs}, results

    @torch.no_grad()
    def validate_imgs(self, epoch, mode='mid'):
        ...
        for batch in tqdm(self.val_img_loader, desc="render val images"):
            ...
            if mode == 'mid':
                results_dir = osp.join(self.save_dir, "results", "rgb_deblur_mid", '%05d'%epoch)
                results.append(rendered["img"].detach())

            os.makedirs(results_dir, exist_ok=True)
            iio.imwrite(
                osp.join(results_dir, f"{frame_name}_img.png"),
                (rendered["img"][0].cpu().numpy() * 255).astype(np.uint8),
            )
            iio.imwrite(
                osp.join(results_dir, f"{frame_name}_img_gt.png"),
    ...
    def validate_imgs_with_optimization(self):
        guru.info("rendering validation images with test time optimization...")
        if self.val_img_loader is None:
            return
        ...
        for batch in tqdm(self.val_img_loader, desc="render val images"):
            ...
            transR = torch.zeros((3, 3)).cuda()
            transR[0, 0] = 1.
            transR[1, 1] = 1.
            transR[2, 2] = 1.
            transT = torch.zeros(3, 1).cuda()

            transR = torch.nn.Parameter(transR.requires_grad_(True)).cuda()
            transT = torch.nn.Parameter(transT.requires_grad_(True)).cuda()
            optimizer = torch.optim.Adam([{'params': [transR, transT],
                                        'lr': 1e-2, "name": "camera pose refine"}])
            scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=500, eta_min=1e-4)
            
            first_loss = 0.
            for iter in range(500):
                # w2c作用可学习的pose
                transR = transR.unsqueeze(0)
                transT = transT.unsqueeze(0)
    
                w2c_trans = torch.zeros_like(w2c)
                w2c_trans[:, :3, :3] = transR @ w2c[:, :3, :3]
                w2c_trans[:, :3, 3:] = transT + w2c[:, :3, 3:]
                rendered = self.model.render(t, w2c_trans, K, img_wh, return_depth=True)

                loss = l1_loss(rendered["img"], img)
                loss.backward()
                ...
                if iter == 0:
                    first_loss = loss
                print(f"{iter} first_loss={first_loss:03f} loss={loss:03f}", end='\r')

            rendered["img"] = rendered["img"].detach()
            # Compute metrics.
            ...
            # Dump results.
            results_dir = osp.join(self.save_dir, "results", "rgb_test_optim")
            os.makedirs(results_dir, exist_ok=True)
            iio.imwrite(
            ...
            iio.imwrite(
                osp.join(results_dir, f"{frame_name}_gt.png"),
                (img[0].cpu().numpy() * 255).astype(np.uint8),
            )
