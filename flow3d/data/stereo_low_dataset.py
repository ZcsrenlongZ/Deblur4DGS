...
class StereoLowDataset(BaseDataset):
    def __init__(
        self,
        data_dir: str,
        start: int = 0,
        ...
        **_,
        ...
        if self.camera_type == "refined":
            ...
            self.Ks[:, :2] /= 2.5 # for low resolution
            self.Ks[:, :2] /= factor
    ...
    def get_video_dataset(self) -> Dataset:
        return StereoLowDatasetVideoView(self)
...
class StereoLowDatasetVideoView(Dataset):
    """Return a dataset view of the video trajectory."""

    def __init__(self, dataset: StereoLowDataset):
        super().__init__()
