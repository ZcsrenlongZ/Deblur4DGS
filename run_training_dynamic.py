...
def main(cfg: TrainConfig):
    ...
    for jj in range(0, len(slides_dict.keys())):
        for ss in range(0, len(scales)):
            ...
            ckpt_path = os.path.join("/".join(cfg.work_dir.split('/')[0:-1]), '0023', 'ckpts/static/499.ckpt')
            ...
            batches3 = []
            files = glob.glob(
                os.path.join("/".join(cfg.work_dir.split('/')[0:-1]), '0023', 'results/rgb_deblur_mid/00499/*_img.png')
            )
            ...
            if ss == 0:
                ...
                files = glob.glob(
                    os.path.join("/".join(cfg.work_dir.split('/')[0:-2]),
                                scales[ss-1],
                                cfg.work_dir.split('/')[-1], 
                                'results/rgb_deblur_mid/00199/*_img.png')
                )
                files = sorted(files, key=lambda x: int(x.split('/')[-1].split('.')[0].split('_')[0]))
                files = files[::2]
